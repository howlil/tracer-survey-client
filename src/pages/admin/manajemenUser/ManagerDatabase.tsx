...
function ManagerDatabase() {
  ...
  const positions = React.useMemo(() => {
    const uniquePositions = [...new Set(managers.map(m => m.position))].sort()
  ...
  React.useEffect(() => {
    ...
    if (searchQuery.trim()) {
      filtered = filtered.filter(manager => 
        manager.respondent.fullName.toLowerCase().includes(searchQuery.toLowerCase()) ||
        manager.respondent.email.toLowerCase().includes(searchQuery.toLowerCase())
      )
    }
    
    // Filter by company
    if (selectedCompany && selectedCompany !== "all") {
      filtered = filtered.filter(manager => manager.company === selectedCompany)
    }
    
    // Filter by position
    if (selectedPosition && selectedPosition !== "all") {
      filtered = filtered.filter(manager => manager.position === selectedPosition)
    }
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          ...
          <Card>
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-muted-foreground">Ditampilkan</p>
                  <p className="text-2xl font-bold text-foreground">{filteredManagers.length}</p>
                </div>
              ...
              <div className="space-y-6">
                ...
                <div className="flex items-center justify-between pt-4 border-t border-border/30">
                  <div className="text-sm text-muted-foreground">
                    Menampilkan <span className="font-semibold text-foreground">{filteredManagers.length}</span> dari{' '}
                    <span className="font-semibold text-foreground">{managers.length}</span> manager
                  </div>
                  <Button
            ...
            <Table>
              ...
              <TableBody>
                {currentManagers.map((manager) => (
                  <TableRow key={manager.id} className="border-border/30 hover:bg-muted/30 transition-colors">
