...
const KelolaTemplateEmailEditor: React.FC = () => {
  ...
  const [emailBlocks, setEmailBlocks] = useState<Array<{
    id: string;
    type: string;
    content: string;
  ...
  useEffect(() => {
    if (isEdit && id) {
      ...
      setFormData({
        ...
        bodyText: mockTemplate.bodyText,
        bodyHtml: mockTemplate.bodyHtml
      });
    } else {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (showSuggestions) {
        ...
        if (!target.closest('.suggestion-dropdown') && !target.closest('[data-block-id]')) {
  ...
  const handleSaveTemplate = async () => {
    setLoading(true);
    try {
      // Generate HTML from visual blocks
  ...
  const getDefaultBlockContent = (type: string) => {
    switch (type) {
      ...
      case 'paragraph':
        return { content: 'Halo {{user.name}}, selamat datang di sistem Tracer Study Universitas Andalas. Kami senang Anda bergabung dengan kami untuk melacak perkembangan karir alumni.' };
      case 'button':
        return { content: 'Mulai Survey', link: '{{survey.link}}' };
      case 'image':
        return { content: 'https://via.placeholder.com/400x200/14b8a6/ffffff?text=UNIVERSITAS+ANDALAS', alt: 'Logo Universitas Andalas' };
      case 'divider':
        return { content: '' };
      case 'footer':
        return { content: 'Terima kasih atas partisipasi Anda.\n\nTim Tracer Study\nUniversitas Andalas' };
      default:
        return { content: '' };
    }
  };
  ...
  const handleBlockInput = (blockId: string, field: string, value: string, event: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    const cursorPos = (event.target as HTMLInputElement | HTMLTextAreaElement).selectionStart || 0;
    ...
    if (value.includes('{{') && cursorPos > 0) {
      ...
      if (lastOpenBrace !== -1) {
        ...
        if (!hasClosingBrace) {
          // Show suggestions
          const rect = (event.target as HTMLInputElement | HTMLTextAreaElement).getBoundingClientRect();
  ...
  const handleBlockKeyUp = (blockId: string, value: string, event: React.KeyboardEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    const cursorPos = (event.target as HTMLInputElement | HTMLTextAreaElement).selectionStart || 0;
    setCursorPosition(cursorPos);
    setActiveBlockId(blockId);
    ...
    if (activeBlockId === 'subject') {
      ...
      if (lastOpenBrace !== -1) {
        ...
        setTimeout(() => {
          ...
          const input = document.querySelector(`[data-block-id="subject"]`) as HTMLInputElement;
          if (input) {
            input.focus();
      ...
      if (lastOpenBrace !== -1) {
        ...
        setTimeout(() => {
          const newCursorPos = lastOpenBrace + variable.length + 4; // 4 for "{{" and "}}"
          const input = document.querySelector(`[data-block-id="${activeBlockId}"]`) as HTMLInputElement | HTMLTextAreaElement;
          if (input) {
            input.focus();
            input.setSelectionRange(newCursorPos, newCursorPos);
          }
  ...
  const generateHtmlFromBlocks = () => {
    const htmlBlocks = emailBlocks.map(block => {
      switch (block.type) {
        case 'header':
        ...
        case 'button':
          return `<div style="text-align: center; margin: 40px 0;">
            <a href="${block.link || '#'}" style="display: inline-block; background: linear-gradient(135deg, #14b8a6 0%, #10b981 100%); color: #ffffff; text-decoration: none; padding: 16px 32px; border-radius: 12px; font-weight: 600; font-size: 16px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; box-shadow: 0 4px 12px rgba(20, 184, 166, 0.3); transition: all 0.3s ease; border: none; cursor: pointer;">${block.content}</a>
          </div>`;
        case 'image':
          return `<div style="text-align: center; margin: 32px 0;">
            <img src="${block.content}" alt="${block.alt || ''}" style="max-width: 100%; height: auto; border-radius: 12px; box-shadow: 0 8px 25px rgba(0, 0, 0, 0.15); border: 2px solid #f0fdfa;" />
          </div>`;
        case 'divider':
          return `<div style="text-align: center; margin: 40px 0;">
  ...
  return (
    <AdminLayout>
        ...
        <div className='flex-1 flex overflow-hidden'>
          ...
          <div className='w-80 border-r bg-gray-50 overflow-y-auto'>
            <div className='p-4'>
              ...
              <div className='space-y-4'>
                <h3 className='font-semibold text-sm text-gray-700'>Blok Email</h3>
                
                {/* Email Blocks */}
                <div className='space-y-2'>
                  <h4 className='text-sm font-medium text-gray-600'>Tambahkan Blok</h4>
                  <div className='grid grid-cols-2 gap-2'>
                    <Button variant='outline' size='sm' onClick={() => addEmailBlock('header')}>
                      <Type className='mr-1 h-3 w-3' />
                      Header
                    </Button>
                    <Button variant='outline' size='sm' onClick={() => addEmailBlock('paragraph')}>
                      <Square className='mr-1 h-3 w-3' />
                      Paragraf
                    </Button>
                    <Button variant='outline' size='sm' onClick={() => addEmailBlock('button')}>
                      <Circle className='mr-1 h-3 w-3' />
                      Tombol
                    </Button>
                    <Button variant='outline' size='sm' onClick={() => addEmailBlock('image')}>
                      <Image className='mr-1 h-3 w-3' />
                      Gambar
                    </Button>
                    <Button variant='outline' size='sm' onClick={() => addEmailBlock('divider')}>
                      <Triangle className='mr-1 h-3 w-3' />
                      Pemisah
                    </Button>
                    <Button variant='outline' size='sm' onClick={() => addEmailBlock('footer')}>
                ...
                {/* Quick Templates */}
                <div className='space-y-2'>
                  <h4 className='text-sm font-medium text-gray-600'>Template Cepat</h4>
                  <Button
                    variant='default'
                    size='sm'
                    onClick={() => {
                      setEmailBlocks([
                        { id: '1', type: 'header', content: 'Selamat Datang di Sistem Tracer Study' },
                        { id: '2', type: 'paragraph', content: 'Halo {{user.name}}, selamat datang di sistem Tracer Study Universitas Andalas. Kami senang Anda bergabung dengan kami untuk melacak perkembangan karir alumni.' },
                        { id: '3', type: 'paragraph', content: 'Silakan lengkapi survey berikut untuk membantu kami meningkatkan kualitas pendidikan dan layanan kepada mahasiswa.' },
                        { id: '4', type: 'button', content: 'Mulai Survey', link: '{{survey.link}}' },
                        { id: '5', type: 'footer', content: 'Terima kasih atas partisipasi Anda.\n\nTim Tracer Study\nUniversitas Andalas' }
                      ]);
                    }}
                    className='w-full'
                  >
                  ...
                  <Button
                    ...
                    onClick={() => {
                      setEmailBlocks([
                        { id: '1', type: 'header', content: '⏰ Pengingat Survey Tracer Study' },
                        { id: '2', type: 'paragraph', content: 'Halo {{user.name}},' },
                        { id: '3', type: 'paragraph', content: 'Kami ingin mengingatkan bahwa survey Tracer Study Anda belum lengkap. Survey ini sangat penting untuk membantu kami meningkatkan kualitas pendidikan di Universitas Andalas.' },
                        { id: '4', type: 'paragraph', content: 'Survey akan berakhir pada {{survey.deadline}}. Silakan lengkapi survey Anda segera.' },
                        { id: '5', type: 'button', content: 'Lengkapi Survey Sekarang', link: '{{survey.link}}' },
                        { id: '6', type: 'footer', content: 'Terima kasih atas perhatiannya.\n\nTim Tracer Study\nUniversitas Andalas' }
                      ]);
                    }}
                    className='w-full'
                  >
                  ...
                  <Button
                    ...
                    onClick={() => {
                      setEmailBlocks([
                        { id: '1', type: 'header', content: '🎓 Survey Alumni Universitas Andalas' },
                        { id: '2', type: 'paragraph', content: 'Halo {{user.name}}, sebagai alumni Universitas Andalas, kami membutuhkan feedback Anda untuk meningkatkan kualitas pendidikan dan layanan kepada mahasiswa.' },
                        { id: '3', type: 'paragraph', content: 'Survey ini akan membantu kami memahami perkembangan karir alumni dan mengevaluasi program studi yang telah Anda ikuti.' },
                        { id: '4', type: 'image', content: 'https://via.placeholder.com/400x200/14b8a6/ffffff?text=ALUMNI+UNAND', alt: 'Alumni Universitas Andalas' },
                        { id: '5', type: 'button', content: 'Isi Survey Alumni', link: '{{survey.link}}' },
                        { id: '6', type: 'footer', content: 'Kontribusi Anda sangat berarti bagi kemajuan Universitas Andalas.\n\nRektorat Universitas Andalas' }
                      ]);
                    }}
                    className='w-full'
                  >
                ...
                {emailBlocks.length > 0 && (
                  <div className='space-y-2'>
                    <h4 className='text-sm font-medium text-gray-600'>Blok yang Ditambahkan</h4>
                    <div className='space-y-1'>
                      {emailBlocks.map((block, index) => (
                        <div key={block.id} className='flex items-center justify-between p-2 bg-white border rounded text-xs'>
                ...
                <CardContent className='space-y-4'>
                  ...
                  <div className='space-y-2'>
                    ...
                    <Input
                      ...
                      onKeyUp={(e) => handleBlockKeyUp('subject', e.currentTarget.value, e)}
                    ...
                    <div className='border rounded-lg p-4 bg-gray-50 min-h-96'>
                      {emailBlocks.length === 0 ? (
                        <div className='text-center text-gray-500 py-8'>
                          ...
                          <p>Belum ada blok email. Klik tombol di panel kiri untuk menambahkan blok.</p>
                        </div>
                      ) : (
                        <div className='space-y-4'>
                          {emailBlocks.map((block) => (
                            <div key={block.id} className='bg-white border rounded-lg p-4 hover:shadow-md transition-shadow'>
                              ...
                              {block.type === 'header' && (
                                <Input
                                  ...
                                  value={block.content}
                                  onChange={(e) => handleBlockInput(block.id, 'content', e.target.value, e)}
                                  onKeyUp={(e) => handleBlockKeyUp(block.id, e.currentTarget.value, e)}
                              ...
                              {block.type === 'button' && (
                                <div className='space-y-2'>
                                  ...
                                  <Input
                                    ...
                                    value={block.link || ''}
                                    onChange={(e) => handleBlockInput(block.id, 'link', e.target.value, e)}
                                    onKeyUp={(e) => handleBlockKeyUp(block.id, e.currentTarget.value, e)}
                              ...
                              {block.type === 'image' && (
                                <div className='space-y-2'>
                                  ...
                                  <Input
                                    ...
                                    value={block.alt || ''}
                                    onChange={(e) => handleBlockInput(block.id, 'alt', e.target.value, e)}
                                    onKeyUp={(e) => handleBlockKeyUp(block.id, e.currentTarget.value, e)}
                              ...
                              {block.type === 'divider' && (
                                <div className='border-t border-gray-300 py-2'>
                                  <span className='text-sm text-gray-500'>Pemisah</span>
                                </div>
                              )}
                            </div>
                          ))}
          ...
          <div className='w-96 border-l bg-gray-50 overflow-y-auto'>
            <div className='p-4'>
              {/* Subject Preview */}
              <div className='mb-4'>
                <h4 className='text-xs font-medium text-gray-600 mb-2'>Subject</h4>
                <div className='p-3 bg-white border rounded text-sm'>
                  {formData.subject ? replaceEmailVariables(formData.subject, {
                    'user.name': 'John Doe',
                    'survey.deadline': '31 Desember 2024',
                    'admin.name': 'Admin Sistem'
                  }) : 'Belum ada subject'}
              ...
              <div>
                <div className='border rounded bg-white p-4 max-h-96 overflow-y-auto'>
                  {emailBlocks.length > 0 ? (
                    ...
                    <div className='text-sm text-gray-500 italic'>Belum ada content</div>
                  )}
                </div>
              </div>
            </div>
      ...
      {showSuggestions && createPortal(
        ...
        >
          <div className='p-2'>
            <div className='text-xs font-medium text-gray-500 mb-2 px-2'>Variable yang tersedia:</div>
            {EMAIL_VARIABLES.map((variable) => (
              ...
              >
                <div className='font-mono text-teal-600 font-semibold'>{variable.key }</div>
                <div className='text-xs text-gray-600 mt-1'>{variable.description}</div>
                <div className='text-xs text-gray-500 mt-1'>Contoh: {variable.example}</div>
              </div>
            ))}
          </div>
        </div>,
